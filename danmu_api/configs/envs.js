...
export class Envs {
  ...
  static ALLOWED_PLATFORMS = ['qiyi', 'bilibili1', 'imgo', 'youku', 'qq', 'migu', 'renren', 'hanjutv', 'sohu', 'leshi', 'xigua', 'maiduidui', 'aiyifan', 'hongguo', 'dandan', 'bahamut', 'animeko', 'acfun', 'niconico', 'custom']; // 全部源允许的播放平台
  static ALLOWED_SOURCES = ['360', 'vod', 'tmdb', 'douban', 'tencent', 'youku', 'iqiyi', 'imgo', 'bilibili', 'migu', 'renren', 'hanjutv', 'sohu', 'leshi', 'xigua', 'maiduidui', 'aiyifan', 'hongguo', 'dandan', 'bahamut', 'animeko', 'acfun', 'niconico', 'custom']; // 允许的源
  static MERGE_ALLOWED_SOURCES = ['tencent', 'youku', 'iqiyi', 'imgo', 'bilibili', 'migu', 'renren', 'hanjutv', 'sohu', 'leshi', 'xigua', 'maiduidui', 'aiyifan', 'hongguo', 'dandan', 'bahamut', 'animeko', 'acfun', 'niconico']; // 允许的源合并
...
如果没有找到合适的匹配，返回：
{
  ...
  static load(env = {}) {
    ...
    const envVarConfig = {
      ...
      'SEARCH_EARLY_RETURN_MS': { category: 'source', type: 'number', description: 'Node 多源搜索获得足够候选后的最早返回时间（毫秒），默认2500', min: 500, max: 15000 },
      'SEARCH_EARLY_RETURN_MIN_RESULTS': { category: 'source', type: 'number', description: '自适应提前返回所需的最少候选数，默认1', min: 1, max: 100 },
      'SEARCH_EARLY_RETURN_MIN_SOURCES': { category: 'source', type: 'number', description: '自适应提前返回前至少已完成的来源数，默认10', min: 1, max: 30 },
      ...
      'NIPAPLAY_APP_ID': { category: 'source', type: 'text', description: '弹弹play开放平台 App ID，仅用于 NipaPlay 302 关联弹幕；需自行合法申请' },
      'NIPAPLAY_APP_SECRET': { category: 'source', type: 'text', description: '弹弹play开放平台 App Secret，仅用于 NipaPlay 302 关联弹幕；需自行合法申请' },
    ...
    return {
      ...
      searchEarlyReturnMs: this.get('SEARCH_EARLY_RETURN_MS', 2500, 'number'), // Node 搜索获得有效候选后的最早返回时间
      searchEarlyReturnMinResults: this.get('SEARCH_EARLY_RETURN_MIN_RESULTS', 1, 'number'), // 提前返回所需的最少候选数
      searchEarlyReturnMinSources: this.get('SEARCH_EARLY_RETURN_MIN_SOURCES', 10, 'number'), // 提前返回前至少已完成的来源数
      ...
      nipaplayAppId: this.get('NIPAPLAY_APP_ID', '', 'string', true), // 弹弹play开放平台 App ID，仅运行时注入
      nipaplayAppSecret: this.get('NIPAPLAY_APP_SECRET', '', 'string', true), // 弹弹play开放平台 App Secret，仅运行时注入
