...
仓库内的模板不包含真实值。VPS 上保留旧版本目录和 systemd 单元备份，升级失败时将 `/opt/oyo-danmu-api/current` 恢复到前一版本并重启 `danmu-api.service`。运行时搜索、集号和收藏缓存保存在 `/opt/oyo-danmu-api/shared/cache`，发布脚本会在首次升级时迁移当前版本的 `.cache`，后续版本通过符号链接复用，避免重启丢失映射。
